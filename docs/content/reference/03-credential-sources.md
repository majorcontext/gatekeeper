...
When the upstream server returns `401 Unauthorized`, `403 Forbidden`, or `429 Too Many Requests`, gatekeeper invalidates every credential that was injected into that request, so the next request re-resolves instead of replaying the cached value. This applies to any credential source that supports invalidation, not just `token-exchange`. A `429` can indicate that the selected backing credential exhausted its allowance while its token remains valid. Other statuses are left alone — a 5xx says nothing about the credential's validity, and these three statuses can also cover cases where resolving again returns the same value.

For `token-exchange`, invalidation evicts the cached token for that subject (and actor) so the next request performs a fresh exchange. Evictions are rate-limited per subject/actor key to a 10-second cooldown: repeated 401/403/429 responses for the same key within 10 seconds are no-ops after the first eviction. This bounds how often a client looping on a failing request can force STS calls; the trade-off is that a genuinely changed backing credential can take up to 10 seconds to be picked up after the first eviction.
