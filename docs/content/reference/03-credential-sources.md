...
Maximum time to cache an exchanged token, expressed as a Go duration.

- **Type:** `string`
- **Required:** No
- **Default:** `"1m"`

The effective TTL is the earlier of `cache_ttl`, the STS `expires_in`, and the hard one-minute ceiling. Values above one minute are capped. Set `cache_ttl: "0"` to disable caching when the STS dynamically selects backing state that can change while the subject and actor remain constant. Concurrent requests for the same key are still coalesced with singleflight.
...
Exchanged tokens are cached per subject (and actor, if present) by default. The STS `expires_in` controls token lifetime while `cache_ttl` controls Gatekeeper's cache ceiling; both are capped at one minute. If the STS omits `expires_in` or returns a non-positive value, `cache_ttl` is used directly. `cache_ttl: "0"` disables caching. Concurrent requests for the same subject are coalesced into a single STS call via singleflight.
