...
Only meaningful with `subject_from: proxy-auth`; a `subject_header`-mode resolver never compares against it. Pick a value that cannot collide with a real subject your deployment could see — `"-"` is a safe default.
