...
- **Post-release fixes to the v0.15.2 host-key lookup** — a review of the shipped lookup surfaced four defects, all fixed here. *Request logs no longer capture resolver-stripped credentials*: v0.15.2's plain-HTTP handler snapshotted request headers for logging before the credential resolver ran, so a subject-identity token the resolver removes (e.g. a token-exchange subject header) was written verbatim to request logs; the snapshot now happens after resolution, and credential values never reach the log. *Outranked resolvers no longer stall requests*: when a better-matched static credential decides a request, v0.15.2 still ran the wildcard-matched resolver synchronously and discarded its result — an unrelated IdP outage added a full STS timeout to every request on the static-credential host. The resolver is now skipped entirely; because a resolver may also sanitize the request, resolvers that remove headers declare them at registration via the new `SetCredentialResolverWithStripHeaders`, and the proxy removes the declared headers itself when it skips the resolver (the built-in token-exchange wiring declares its `subject_header`). *Port-pinned keys fire regardless of how the client spells the URL*: the plain-HTTP path passed the request host with a port only when the client literally included one, so a `:80`-pinned key matched `http://host:80/` but not `http://host/`; the scheme-default port is now made explicit before lookup, and a bracketed portless IPv6 lookup host (`[::1]`) matches keys stored in canonical form (`::1`). *Cross-map ranking gained the remaining in-map tie rule*: a port-pinned exact or case-fold key now outranks a port-less one across maps too (a port-pinned static credential is no longer shadowed by a bare-host resolver), via one specificity comparator shared by in-map and cross-map selection
