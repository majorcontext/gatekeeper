...
- **Post-release fixes to the v0.15.2 host-key lookup** — a deep review of the shipped lookup surfaced five defects, all fixed here. *Request logs no longer capture resolver-stripped credentials*: the plain-HTTP handler snapshotted request headers for logging before the credential resolver ran, so a subject-identity token the resolver removes (e.g. a token-exchange subject header) was written verbatim to request logs; the snapshot now happens after resolution, and credential values never reach the log. *Outranked resolvers no longer stall requests*: when a better-matched static credential decides a request, the proxy previously still ran the wildcard-matched resolver synchronously and discarded its result — an unrelated IdP outage added a full STS timeout to every request on the static-credential host. The resolver is now skipped entirely; because a resolver may also sanitize the request, resolvers that remove headers declare them at registration via the new `SetCredentialResolverWithStripHeaders`, and the proxy removes the declared headers itself when it skips the resolver (the built-in token-exchange wiring declares its `subject_header`). *`SetCredentialResolver(host, nil)` is an opt-out again*: the v0.15.2 lookup skipped nil entries, so a broader wildcard resolver fired for a host an embedder had explicitly disabled; the nil entry now matches and terminates the lookup, like the companion maps' empty-entry opt-outs. *Port-pinned keys now fire on the data paths*: the CONNECT-interception and plain-HTTP handlers passed a port-stripped host to every lookup, so v0.15.2's advertised port-pinned keys never matched there; both paths now pass the port-bearing request target — with the scheme-default port made explicit on plain HTTP, so a `:80`-pinned key fires for `http://host/` too — and a bracketed portless IPv6 lookup host (`[::1]`) matches keys stored in canonical form (`::1`). *Policy is checked before resolution on plain HTTP* (matching the interception path), so a policy-denied client can no longer trigger a resolver's external round trips via fabricated wildcard-matched hosts, and receives the 407 policy denial rather than a resolver 502. Cross-map ranking also gained the in-map tie rules: a verbatim-cased match outranks a case-fold match and a port-pinned key outranks a port-less one, via one shared specificity comparator
